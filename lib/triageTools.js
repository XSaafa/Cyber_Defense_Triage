...
export async function scanIOC({ ioc_value, ioc_type }) {
  const VALID_IOC_TYPES = ['ip', 'domain', 'hash', 'url', 'email']

  // Guard: reject invalid ioc_type (e.g. 'unknown', MITRE IDs, CVE IDs)
  if (!VALID_IOC_TYPES.includes(ioc_type)) {
    return `## scan_ioc Error\n\n` +
      `**Invalid ioc_type:** \`${ioc_type}\`\n\n` +
      `Only these types are valid: ${VALID_IOC_TYPES.join(', ')}\n\n` +
      `**Note:** MITRE ATT&CK technique IDs (T1xxx) and CVE IDs are NOT IOCs and cannot be scanned. ` +
      `Use \`check_cve\` for CVE lookups. MITRE technique IDs should only be referenced in your report.`
  }
